...
#error Invalid timer selected!
	...
	timer_set_load_prepare(configTIMER_CLOCK_HZ / configTICK_RATE_HZ);
	timer_enable_intr_prepare();

	/* Configure the VIC to service IRQ4 (triggered by the timer) properly */
	pic_registerIrq(5, (void *)&vTickISR, PIC_MAX_PRIORITY);

	/* Enable servicing of IRQ4 */
	pic_enableInterrupt(5);
...
{
	return configAPB_CLOCK_HZ;
}

void vAssertCalled(const char * const pcFileName, unsigned long ulLine, const char *func)
{
	...
	taskENTER_CRITICAL();
	{
        ...
        value. */
		print_msg("Assert, Line:%d, File:%s Func:%s\n", ulLine, pcFileName, func);
		while(1);
	}
}
