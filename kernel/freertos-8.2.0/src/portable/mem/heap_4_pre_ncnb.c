...
/* Allocate the memory for the heap. */
	...
	extern uint8_t ucHeap_pre_ncnb[ configHEAP_PRE_NCNB ];
#else

	static uint8_t ucHeap_pre_ncnb[ configHEAP_PRE_NCNB ];
...
static size_t xTotalHeapSize_pre_ncnb = 0U;
static size_t xUsedBytesByMod_pre_ncnb[MODULE_DEF_MAX] = {0U};
static size_t xMaxEverUsedBytesByMod_pre_ncnb[MODULE_DEF_MAX] = {0U};
		...
		if( ( xWantedSize & xBlockAllocatedBit ) == 0 )
		{
			...
			if( ( xWantedSize > 0 ) && ( xWantedSize <= xFreeBytesRemaining ) )
			{
				...
				if( pxBlock != pxEnd )
				{
					...
					xUsedBytesByMod_pre_ncnb[Module] += pxBlock->xBlockSize;
					if(xUsedBytesByMod_pre_ncnb[Module]> xMaxEverUsedBytesByMod_pre_ncnb[Module])
					{
						xMaxEverUsedBytesByMod_pre_ncnb[Module] = xUsedBytesByMod_pre_ncnb[Module];
					}
					else
					{
						mtCOVERAGE_TEST_MARKER();
		...
		if( ( pxLink->xBlockSize & xBlockAllocatedBit ) != 0 )
		{
			if( pxLink->pxNextFreeBlock == NULL )
			{
				...
				vTaskSuspendAll();
				{
					...
					{
						xUsedBytesByMod_pre_ncnb[module_id] -= pxLink->xBlockSize;
					}
					else
					{
						mtCOVERAGE_TEST_MARKER();
...
void *pvPortRealloc_pre_ncnb(void *pv, size_t size, size_t Module)
{
	...
	if( pv != NULL ) {
		...
		pvReturn = pvPortMalloc_pre_ncnb(size, Module);
...
{
	return xTotalHeapSize_pre_ncnb;
}
/*-----------------------------------------------------------*/
...
{
	return xUsedBytesByMod_pre_ncnb;
}
...
{
	return xMaxEverUsedBytesByMod_pre_ncnb;
}
...
uint32_t ulAddress;
size_t xTotalHeapSize = configHEAP_PreNCNB_SIZE;

	/* Ensure the heap starts on a correctly aligned boundary. */
	ulAddress = ( uint32_t ) ucHeap_pre_ncnb;
...
//printf("ulAddress=0x%x\n", ulAddress);
	if( ( ulAddress & portBYTE_ALIGNMENT_MASK ) != 0 )
	{
		...
		xTotalHeapSize -= ulAddress - ( uint32_t ) ucHeap_pre_ncnb;
	}

	/* Init xTotalHeapSize_cb */
	xTotalHeapSize_pre_ncnb = xTotalHeapSize;
