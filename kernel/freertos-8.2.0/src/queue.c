...
int8_t *pcAllocatedBuffer;
	...
	pcAllocatedBuffer = ( int8_t * ) pvPortMalloc( sizeof( Queue_t ) + xQueueSizeInBytes, GFP_KERNEL, MODULE_KERNEL );
...
/*-----------------------------------------------------------*/
	...
	Queue_t *pxNewQueue;
		...
		pxNewQueue = ( Queue_t * ) pvPortMalloc( sizeof( Queue_t ), GFP_KERNEL, MODULE_KERNEL );
		if( pxNewQueue != NULL )
		{
