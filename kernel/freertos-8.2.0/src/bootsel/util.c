...
extern volatile unsigned int gl_err_flag; 


unsigned int readmem(volatile unsigned char *raddr, volatile unsigned char *buf, volatile unsigned int nbyte)
{
volatile  unsigned int i;
volatile  unsigned int valbyts = 0;

  for(i=0; i<nbyte; i++){
    if (((volatile unsigned char *)(raddr + i)) < endfileaddr) {
        buf[i] = *((volatile unsigned char*) (raddr + i));
        valbyts++;
    }
    else {
      gl_err_flag = 1;
 ...
 //  fprintf(stderr, "valbyts=%x\n",valbyts);

    ifd += valbyts;
    return valbyts;
}

unsigned int writemem(volatile unsigned char *waddr, volatile unsigned char *buf,volatile unsigned int nbyte)
{
 volatile unsigned int i;

  for(i=0; i<nbyte; i++){
    *((volatile unsigned char*)(waddr + i)) = buf[i];
    ...
    int eof_ok;          /* set if EOF acceptable as a result */
{
 ...
 //   errno = 0;
    ...
    do {
	       len = readmem(ifd, (unsigned char*)inbuf+insize, INBUFSIZ-insize);
...
void flush_outbuf()
{
    ...
    write_buf(ofd, (unsigned char *)outbuf, outcnt);
...
void flush_window()
{
    ...
    if (!test) {
	write_buf(ofd, (unsigned char *)window, outcnt);
    }
    bytes_out += (ulg)outcnt;
    outcnt = 0;
}
...
void write_buf(fd, buf, cnt)
    volatile unsigned char       *fd;
    voidp     buf;
   volatile unsigned  cnt;
{
   volatile unsigned  n;

    while ((n = writemem(fd, buf, cnt)) != cnt) {
	     // if (n == (unsigned)(-1)) {
	     //     write_error();
	     // }
	   cnt -= n;
