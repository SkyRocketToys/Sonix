...
function generate_kernel_type()
{
  ...
  par_how=${para_arr[7]}
	...
	if [ "$par_num" = "5" ]
	then
		echo "ifeq (\$(shell echo \$(MODULE_ONLY) | tr \"[:upper:]\" \"[:lower:]\" ), yes)" >> $par_fil	
	...
	if [ "$par_dir" = ".git" ]
	then
		echo "	\$(MAKE) -C \$(PRJ_DIR)/$par_whe all OBJ_DIR=\$(PRJ_OBJ_DIR)/$par_whe " >> $par_fil
		echo "	\$(MAKE) -C \$(PRJ_DIR)/$par_whe install INSTALL_DIR=\$(PRJ_DIR)/$par_whe/target OBJ_DIR=\$(PRJ_OBJ_DIR)/$par_whe" >> $par_fil
		echo "	if [ -d \$(PRJ_DIR)/$par_whe/target ]; then \\" >> $par_fil
		echo "		ln -sf \$(PRJ_DIR)/$par_whe/target/include/* \$(KERNEL_INC_DIR); \\" >> $par_fil
		echo "		ln -sf \$(PRJ_DIR)/$par_whe/target/lib/* \$(KERNEL_LIB_DIR); \\" >> $par_fil	
		echo "	fi" >> $par_fil
		echo "ifeq (\$(CONFIG_RESCUE_SYSTEM), y)" >> $par_fil
		if [ "$cur_rescue_flag" = "compile_rescue" ]
		then	
		echo "	if [ -d \$(PRJ_DIR)/$par_whe/target ]; then \\" >> $par_fil
		echo "		ln -sf \$(PRJ_DIR)/$par_whe/target/include/* \$(KERNEL_INC_DIR_RESCUE); \\" >> $par_fil
		echo "		ln -sf \$(PRJ_DIR)/$par_whe/target/lib/* \$(KERNEL_LIB_DIR_RESCUE); \\" >> $par_fil	
		echo "	fi" >> $par_fil
		fi
		echo "endif" >> $par_fil
	else
	...
	# if [ "$cur_rescue_flag" = "compile_rescue" ]
	# then		
	# 	echo "	if ! [ -d \$(PRJ_OBJ_DIR_RESCUE)/$par_whe ]; then \\" >> $par_fil
	# 	echo "		mkdir -p \$(PRJ_OBJ_DIR_RESCUE)/$par_whe; \\" >> $par_fil
	# 	echo "	fi;" >> $par_fil
	# 	echo "	chmod 777 \$(PRJ_OBJ_DIR)/$par_whe/*" >> $par_fil
	# 	echo "	cp -av \$(PRJ_OBJ_DIR)/$par_whe/* \$(PRJ_OBJ_DIR_RESCUE)/$par_whe;" >> $par_fil
	# fi
	# echo "endif" >> $par_fil
	...
	if [ "$par_dir" = ".git" ]
	then
		echo "	\$(MAKE) -C \$(PRJ_DIR)/$par_whe clean OBJ_DIR=\$(PRJ_OBJ_DIR)/$par_whe" >> $par_fil
		echo "	\$(MAKE) -C \$(PRJ_DIR)/$par_whe distclean" >> $par_fil
		echo "	rm -rf \$(PRJ_DIR)/$par_whe/target" >> $par_fil
		echo "" >> $par_fil
	else
		echo "" >> $par_fil
	fi
...
###for 20 24 type###
function generate_app_driver_md_type()
{	
	...
	then
		...
		then
			...
			echo "	\$(MAKE) -C \$(PRJ_DIR)/$par_whe install INSTALL_DIR=\$(PRJ_DIR)/$par_whe/target OBJ_DIR=\$(PRJ_OBJ_DIR)/$par_whe " >> $par_fil
			echo "	if [ -d \$(PRJ_DIR)/$par_whe/target ]; then \\" >> $par_fil
			echo "		ln -sf \$(PRJ_DIR)/$par_whe/target/include/* \$(DRIVER_INC_DIR); \\" >> $par_fil
			echo "		ln -sf \$(PRJ_DIR)/$par_whe/target/lib/* \$(DRIVER_LIB_DIR); \\" >> $par_fil	
			echo "	fi" >> $par_fil
			echo "ifeq (\$(CONFIG_RESCUE_SYSTEM), y)" >> $par_fil
			if [ "$cur_rescue_flag" = "compile_rescue" ]
			then	
			echo "	if [ -d \$(PRJ_DIR)/$par_whe/target ]; then \\" >> $par_fil
			echo "		ln -sf \$(PRJ_DIR)/$par_whe/target/include/* \$(DRIVER_INC_DIR_RESCUE); \\" >> $par_fil
			echo "		ln -sf \$(PRJ_DIR)/$par_whe/target/lib/* \$(DRIVER_LIB_DIR_RESCUE); \\" >> $par_fil	
			echo "	fi" >> $par_fil
			fi
			echo "endif" >> $par_fil
		elif [ "$cur_type_flag" = "project" ]
		then
			echo "	\$(MAKE) -C \$(PRJ_DIR)/$par_whe all OBJ_DIR=\$(PRJ_OBJ_DIR)/$par_whe " >> $par_fil
			echo "	\$(MAKE) -C \$(PRJ_DIR)/$par_whe install INSTALL_DIR=\$(PRJ_DIR)/$par_whe/target OBJ_DIR=\$(PRJ_OBJ_DIR)/$par_whe " >> $par_fil
			echo "	if [ -d \$(PRJ_DIR)/$par_whe/target ]; then \\" >> $par_fil
			echo "		cp -rf \$(PRJ_DIR)/$par_whe/target/* \$(PRJ_IMAGE_DIR); \\" >> $par_fil
			echo "	fi" >> $par_fil			
		elif [ "$cur_type_flag" = "middleware" ]
		then
			echo "	\$(MAKE) -C \$(PRJ_DIR)/$par_whe all OBJ_DIR=\$(PRJ_OBJ_DIR)/$par_whe " >> $par_fil
			echo "	\$(MAKE) -C \$(PRJ_DIR)/$par_whe install INSTALL_DIR=\$(PRJ_DIR)/$par_whe/target OBJ_DIR=\$(PRJ_OBJ_DIR)/$par_whe " >> $par_fil
			echo "	if [ -d \$(PRJ_DIR)/$par_whe/target ]; then \\" >> $par_fil
			echo "		ln -sf \$(PRJ_DIR)/$par_whe/target/include/* \$(MIDDLEWARE_INC_DIR); \\" >> $par_fil
			echo "		ln -sf \$(PRJ_DIR)/$par_whe/target/lib/* \$(MIDDLEWARE_LIB_DIR); \\" >> $par_fil	
			echo "	fi" >> $par_fil
			echo "ifeq (\$(CONFIG_RESCUE_SYSTEM), y)" >> $par_fil
			if [ "$cur_rescue_flag" = "compile_rescue" ]
			then	
			echo "	if [ -d \$(PRJ_DIR)/$par_whe/target ]; then \\" >> $par_fil
			echo "		ln -sf \$(PRJ_DIR)/$par_whe/target/include/* \$(MIDDLEWARE_INC_DIR_RESCUE); \\" >> $par_fil
			echo "		ln -sf \$(PRJ_DIR)/$par_whe/target/lib/* \$(MIDDLEWARE_LIB_DIR_RESCUE); \\" >> $par_fil	
			echo "	fi" >> $par_fil
			fi
			echo "endif" >> $par_fil	
		elif [ "$cur_type_flag" = "network" ]
		then
			echo "	\$(MAKE) -C \$(PRJ_DIR)/$par_whe all OBJ_DIR=\$(PRJ_OBJ_DIR)/$par_whe " >> $par_fil
			echo "	\$(MAKE) -C \$(PRJ_DIR)/$par_whe install INSTALL_DIR=\$(PRJ_DIR)/$par_whe/target OBJ_DIR=\$(PRJ_OBJ_DIR)/$par_whe " >> $par_fil
			echo "	if [ -d \$(PRJ_DIR)/$par_whe/target ]; then \\" >> $par_fil
			echo "		ln -sf \$(PRJ_DIR)/$par_whe/target/include/* \$(NETWORK_INC_DIR); \\" >> $par_fil
			echo "		ln -sf \$(PRJ_DIR)/$par_whe/target/lib/* \$(NETWORK_LIB_DIR); \\" >> $par_fil	
			echo "	fi" >> $par_fil
			echo "ifeq (\$(CONFIG_RESCUE_SYSTEM), y)" >> $par_fil
			if [ "$cur_rescue_flag" = "compile_rescue" ]
			then	
			echo "	if [ -d \$(PRJ_DIR)/$par_whe/target ]; then \\" >> $par_fil
			echo "		ln -sf \$(PRJ_DIR)/$par_whe/target/include/* \$(NETWORK_INC_DIR_RESCUE); \\" >> $par_fil
			echo "		ln -sf \$(PRJ_DIR)/$par_whe/target/lib/* \$(NETWORK_LIB_DIR_RESCUE); \\" >> $par_fil	
			echo "	fi" >> $par_fil
			fi
			echo "endif" >> $par_fil	
		elif [ "$cur_type_flag" = "system" ]
		then
			echo "	\$(MAKE) -C \$(PRJ_DIR)/$par_whe all OBJ_DIR=\$(PRJ_OBJ_DIR)/$par_whe " >> $par_fil
			echo "	\$(MAKE) -C \$(PRJ_DIR)/$par_whe install INSTALL_DIR=\$(PRJ_DIR)/$par_whe/target OBJ_DIR=\$(PRJ_OBJ_DIR)/$par_whe " >> $par_fil
			echo "	if [ -d \$(PRJ_DIR)/$par_whe/target ]; then \\" >> $par_fil
			echo "		ln -sf \$(PRJ_DIR)/$par_whe/target/include/* \$(SYSTEM_INC_DIR); \\" >> $par_fil
			echo "		ln -sf \$(PRJ_DIR)/$par_whe/target/lib/* \$(SYSTEM_LIB_DIR); \\" >> $par_fil	
			echo "	fi" >> $par_fil
			echo "ifeq (\$(CONFIG_RESCUE_SYSTEM), y)" >> $par_fil
			if [ "$cur_rescue_flag" = "compile_rescue" ]
			then	
			echo "	if [ -d \$(PRJ_DIR)/$par_whe/target ]; then \\" >> $par_fil
			echo "		ln -sf \$(PRJ_DIR)/$par_whe/target/include/* \$(SYSTEM_INC_DIR_RESCUE); \\" >> $par_fil
			echo "		ln -sf \$(PRJ_DIR)/$par_whe/target/lib/* \$(SYSTEM_LIB_DIR_RESCUE); \\" >> $par_fil	
			echo "	fi" >> $par_fil
			fi
			echo "endif" >> $par_fil
		else		
	...
	#CODESIZE RBK
	echo "ifeq (\$(code-size),1) " >> $par_fil
		echo "	if [ -d \$(PRJ_DIR)/$par_whe/target/lib ]; then \\" >> $par_fil
			echo "		\$(CODESIZE) -t -d \$(PRJ_DIR)/$par_whe/target/lib/*>>codesize.txt; \\" >> $par_fil	
		echo "	fi; " >> $par_fil	
	echo "endif " >> $par_fil	
...
check_type_flag()
{
	...
	do
		...
		elif [ "$type_temp" = "rootfs" ] || [ "$type_temp" = "driver" ] || [ "$type_temp" = "project" ] || [ "$type_temp" = "bootloader" ] || [  "$type_temp" = "kernel" ] || [  "$type_temp" = "tool" ] || [ "$type_temp" = "middleware" ] || [ "$type_temp" = "rootfs-bs" ] || [ "$type_temp" = "example" ]
		then
			if [ "$type_flag" = "0" ]
			then
				type_flag=1
...
function all_files_generate()
{
	...
	do
		...
		if [ "$cur_type_flag" = "driver" ] || [ "$cur_type_flag" = "project" ] || [ "$cur_type_flag" = "network" ] || [ "$cur_type_flag" = "middleware" ] 
		then
			para_arr[5]=$git_exist
			para_arr[6]=$type
			para_arr[7]=$which_depend_type
			generate_app_driver_md_type ${para_arr[@]}
		fi
