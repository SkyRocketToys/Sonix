...
/* SNX Add N/e/d and its length to its struture */
CYASSL_LOCAL int SnxGetInt(mp_int* mpi, const byte* input, int length)
{
    if (mp_init(mpi) != MP_OKAY)
        return MP_INIT_E;

    if (mp_read_unsigned_bin(mpi, (byte*)input, length) != 0) {
        mp_clear(mpi);
        return ASN_GETINT_E;
    }
...
int SnxRsaPrivateKeyDecode(RsaKey* key, const byte* moduls, int moduls_len,
						const byte* exp, int exp_len, const byte* pri_exp,
						int pri_exp_len)
{
	key->type = RSA_PRIVATE;

    if (SnxGetInt(&key->n,  moduls, moduls_len) < 0 ||
        SnxGetInt(&key->e,  exp, exp_len) < 0 ||
        SnxGetInt(&key->d,  pri_exp, pri_exp_len) < 0 )
		return ASN_RSA_KEY_E;
