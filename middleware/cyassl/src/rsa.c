...
int SnxRsaGenerateSignature(RsaKey* key, const byte* in, word32 inLen, byte* out, 
							word32 outLen, RNG* rng)
{
    int sz, ret;
	...
	if (!rng) {
		if ((ret = RsaPad(in, inLen, out, sz, RSA_BLOCK_TYPE_1, NULL)) != 0 ) {
			return ret;
		}
	}
	else {
		if ((ret = RsaPad(in, inLen, out, sz, RSA_BLOCK_TYPE_2, rng)) != 0 )
			return ret;
	}
