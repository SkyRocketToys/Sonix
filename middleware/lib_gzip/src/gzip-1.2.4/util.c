...
#ifdef FREERTOS
extern long total_in, total_out;

static void dump_buffer(char *p, unsigned l)
{
	unsigned i;
	for (i = 0; i < l; ++i) {
		if (!(p[i] & 0xF)) print_msg_queue("\n");
		print_msg_queue("0x%02x ", p[i]);
	}
	print_msg_queue("\n");
}
...
/*
 * readmem
 * Hack read instead of readmem due to only memory handled here.
 */
int readmem(int fd, char* in, ulg len)
{
#if _DEBUG_VERBOSE
	print_msg("[compr] readmem origin p 0x%08x len %u\n", fd, len);
#endif
	// hack fd as input buffer pointer
	if ((bytes_in + insize + len) > total_in)
		len = total_in - (bytes_in + insize);
	memcpy(in, (void*)fd, len);
	ifd += len;

#if _DEBUG_VERBOSE
	if (len > 4) {
		char *buf = (char*)fd;
		print_msg("[compr] readmem actual p 0x%08x len %u "
				"input [%02x, %02x, %02x, %02x] "
				"output[%02x, %02x, %02x, %02x]\n",
				fd, len,
...
/*
 * writemem
 * Hack write instead of writemem due to only memory handled here.
 */
int writemem(int fd, char* out, unsigned len)
{
#if _DEBUG_VERBOSE
	print_msg("[compr] writemem origin p 0x%08x, len %u\n", fd, len);
#endif
	// hack fd as ouput buffer pointer
	memcpy((void*)fd, out, len);
	ofd += len;

#if _DEBUG_VERBOSE
	print_msg("[compr] writemem actual p 0x%08x, len %u\n", fd, len);
#endif
	return len;
}
...
#if _DEBUG_VERBOSE
    print_msg("[compr] fill_inbuf insize %u\n", insize);
#endif
