...
// TODO: xyd: iconv functions is removed .So implement these functions to convert character
#define printf(x,...)
typedef size_t iconv_tt;
static size_t iconv_(iconv_tt cd,
             char **inbuf, size_t *inbytesleft,
             char **outbuf, size_t *outbytesleft);
 static iconv_tt iconv_open_(const char *tocode, const char *fromcode);
 static iconv_tt iconv_close_(iconv_tt cd);

 
 static iconv_tt iconv_open_(const char *tocode, const char *fromcode){return 0;}
  static iconv_tt iconv_close_(iconv_tt cd){return 0;}
 static size_t iconv_(iconv_tt cd,
             char **inbuf, size_t *inbytesleft,
             char **outbuf, size_t *outbytesleft)
 {
 	iconv_tt size=*inbytesleft>*outbytesleft? *outbytesleft : *inbytesleft;
                                   ...
                                   zbar_image_t *img)
{
  ...
  latin1_cd=iconv_open_("UTF-8","ISO8859-1");
  /*But this one is often used, as well.*/
  sjis_cd=iconv_open_("UTF-8","SJIS");
  /*This is a trivial conversion just to check validity without extra code.*/
  utf8_cd=iconv_open_("UTF-8","UTF-8");
  for(i=0;i<nqrdata;i++)if(!mark[i]){
    ...
    for(j = 0; j < sa_size && !err; j++, sym = &(*sym)->next) {
      *sym = _zbar_image_scanner_alloc_sym(iscn, ZBAR_QRCODE, 0);
      ...
      for(k=0;k<qrdataj->nentries&&!err;k++){
        ...
        switch(entry->mode){
          ...
          case QR_MODE_BYTE:{
            ...
            if(eci<0){
              ...
              if(inleft>=3&&
               in[0]==(char)0xEF&&in[1]==(char)0xBB&&in[2]==(char)0xBF){
				...
				printf("LINE %d :in %d out %d",__LINE__,inleft,outleft);
                err=utf8_cd==(iconv_tt)-1||
                 iconv_(utf8_cd,&in,&inleft,&out,&outleft)==(size_t)-1;
				printf("LINE %d :in %d out %d",__LINE__,inleft,outleft);
                if(!err){
              ...
              for(ei=0;ei<3;ei++)if(enc_list[ei]!=(iconv_tt)-1){
				...
				printf("LINE %d :in %d out %d",__LINE__,inleft,outleft);
                err=iconv_(enc_list[ei],&in,&inleft,&out,&outleft)==(size_t)-1;
				printf("LINE %d :in %d out %d",__LINE__,inleft,outleft);
                if(!err){
                  sa_ntext=out-sa_text;
            ...
            /*We were actually given a character set; use it.*/
              ...
              err=eci_cd==(iconv_tt)-1||
               iconv_(eci_cd,&in,&inleft,&out,&outleft)==(size_t)-1;
			  printf("LINE %d :in %d out %d",__LINE__,inleft,outleft);
          ...
          case QR_MODE_KANJI:{
            ...
            outleft=sa_ctext-sa_ntext;
			printf("LINE %d :in %d out %d",__LINE__,inleft,outleft);
            err=sjis_cd==(iconv_tt)-1||
             iconv_(sjis_cd,&in,&inleft,&out,&outleft)==(size_t)-1;
			printf("LINE %d :in %d out %d",__LINE__,inleft,outleft);
            if(!err)sa_ntext=out-sa_text;
          }break;
      ...
      if(eci<=QR_ECI_GLI1){
        eci=-1;
        if(eci_cd!=(iconv_tt)-1)iconv_close_(eci_cd);
      }
    }
    if(eci_cd!=(iconv_tt)-1)iconv_close_(eci_cd);
    if(!err){
  ...
  if(utf8_cd!=(iconv_tt)-1)iconv_close_(utf8_cd);
  if(sjis_cd!=(iconv_tt)-1)iconv_close_(sjis_cd);
  if(latin1_cd!=(iconv_tt)-1)iconv_close_(latin1_cd);
  free(mark);
  return ntext;
}
