...
#define TALLOC_VERSION_RELEASE 1
#define HAVE_VA_COPY
#define _PUBLIC_
#define MIN(a,b) ((a)<(b)?(a):(b))

#define TALLOC_ABORT(reason) console_printf(reason)
...
static unsigned int talloc_magic = (
	~TALLOC_FLAG_MASK & (
		TALLOC_MAGIC_BASE +
		(TALLOC_VERSION_MAJOR << 24) +
		(TALLOC_VERSION_MINOR << 16) +
		(TALLOC_VERSION_RELEASE << 8)));
...
static void talloc_report_depth_FILE_helper(const void *ptr, int depth, int max_depth, int is_ref, void *_f)
{
	...
	if (is_ref) {
		console_printf("%*sreference to: %s\n", depth*4, "", name);
		return;
	}

	tc = talloc_chunk_from_ptr(ptr);
	if (tc->limit && tc->limit->parent == tc) {
		console_printf("%*s%-30s is a memlimit context"
	...
	if (depth == 0) {
		console_printf("%stalloc report on '%s' (total %6lu bytes in %3lu blocks)\n",
	...
	console_printf("%*s%-30s contains %6lu bytes in %3lu blocks (ref %d) %p\n",
		depth*4, "",
		name,
		(unsigned long)talloc_total_size(ptr),
		(unsigned long)talloc_total_blocks(ptr),
...
static void talloc_report_null(void)
{
	if (talloc_total_size(null_context) != 0) {
		talloc_report(null_context, NULL);
	}
}
...
{
	if (talloc_total_size(null_context) != 0) {
		talloc_report_full(null_context, NULL);
	}
}
#endif
...
_PUBLIC_ void talloc_show_parents(const void *context, FILE *file)
{
	...
	if (context == NULL) {
		console_printf("talloc no parents for NULL\n");
		return;
	}

	tc = talloc_chunk_from_ptr(context);
	console_printf("talloc parents of '%s'\n", __talloc_get_name(context));
	while (tc) {
